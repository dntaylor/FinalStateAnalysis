#ifndef FinalStateAnalysis_DataFormats_FinalState_h
#define FinalStateAnalysis_DataFormats_FinalState_h
...
class FinalState : public pat::PATObject<reco::LeafCandidate> {
  public:
    ...
    const edm::Ptr<FinalStateEvent>& evt() const { return event_; }

    virtual FinalState* clone() const = 0;
   ...
   // return the SVfit computed  mass
    ...
    double zCompatibility(FinalStateProxy& cand) const;
    // and the same thing for a general Lorentz vector. Doesn't check sign.
    double zCompatibility(const FinalState::LorentzVector& p4) const;
...
//        int x=-1, int y=-1, int z=-1) const;
  ...
  private:
    edm::Ptr<FinalStateEvent> event_;
};

#endif /* end of include guard: FinalStateAnalysis_DataFormats_FinalState_h */
