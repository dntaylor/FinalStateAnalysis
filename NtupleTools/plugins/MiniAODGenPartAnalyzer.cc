...
MiniAODGenPartAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    std::cout << " --------------------------------------" << std::endl;
    std::map<int, int> numLeps = { {11, 0}, {13, 0}, {15, 0} };
    
    for(size_t i=0; i<pruned->size();i++){
        if(abs((*pruned)[i].pdgId()) == 23 && (*pruned)[i].status() == 22) {
            const Candidate* Z = &(*pruned)[i];
            std::cout << "------------------------------------------------------------" << std::endl;
            std::cout << "PdgID: " << Z->pdgId() << " pt " << Z->pt() << " eta: " << Z->eta() << " phi: " << Z->phi() << std::endl;
...
//get the pointer to the first survied ancestor of a given packed GenParticle in the prunedCollection 
        ...
        if(abs((*pruned)[i].pdgId()) == 24 && (*pruned)[i].status() == 22) {
            const Candidate* W = &(*pruned)[i];
            std::cout << "------------------------------------------------------------" << std::endl;
            std::cout << "PdgID: " << W->pdgId() << " pt " << W->pt() << " eta: " << W->eta() << " phi: " << W->phi() << std::endl;
            std::cout << "  found daugthers: " << std::endl;
            for(size_t j=0; j<packed->size();j++){
//get the pointer to the first survied ancestor of a given packed GenParticle in the prunedCollection 
                const Candidate * motherInPrunedCollection = (*packed)[j].mother(0) ;
                if(motherInPrunedCollection != nullptr && isAncestor( W , motherInPrunedCollection)){
                    std::cout << "     PdgID: " << (*packed)[j].pdgId() << " pt " << (*packed)[j].pt() << " eta: " << (*packed)[j].eta() << " phi: " << (*packed)[j].phi() << std::endl;
                }
            }
        }
    ...
    for(size_t j=0; j<packed->size();j++) {
        if(abs((*packed)[j].pdgId()) == 13 ||  abs((*packed)[j].pdgId()) == 11 || abs((*packed)[j].pdgId()) == 15)
            numLeps[abs((*packed)[j].pdgId())]++;
    }
    std::cout << std::endl << "Num FS Electrons = " << numLeps[11] << std::endl;
    std::cout << "Num FS Muons = " << numLeps[13] << std::endl;
    std::cout << "Num FS Taus = " << numLeps[15] << std::endl;
